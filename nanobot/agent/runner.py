        ...
        for iteration in range(spec.max_iterations):
            ...
            if response.has_tool_calls:
                ...
                for tool_call, result in zip(response.tool_calls, results):
                    ...
                    has_images = isinstance(normalized, list) and any(
                        isinstance(b, dict) and b.get("type") == "image_url" for b in normalized
                    )
                    tool_message = {
                        "role": "user" if has_images else "tool",
                        "tool_call_id": tool_call.id if not has_images else None,
                        "name": tool_call.name if not has_images else None,
                        "content": normalized,
                    }
